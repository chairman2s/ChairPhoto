...
// ── Indexing speed / parallelism plan ───────────────────────────────────────────

/// Setting key for how hard face-indexing (and other index jobs) may push the machine.
...
impl IndexingSpeed {
    ...
    /// (threads inside one ONNX run) was tuned by measurement on the 24-thread dev machine
    /// (see the I7c commit body). Key finding: the ONNX Runtime intra-op pool already
    /// parallelizes each detect/embed across cores, so stacking many concurrent sessions on
    /// top *oversubscribes* and slows throughput. The win comes from giving a **few**
    /// sessions **generous** intra-op threads (`parallelism × intra ≈ cpus`), which keeps two
    /// photos overlapping (one's preload/IO hides behind the other's compute) while each run
    /// still uses the whole CPU.
    pub fn plan(self, available_cpus: usize) -> IndexingPlan {
        ...
        match self {
            ...
            // session ~half the cores of intra-op threads (2 × cpus/2 ≈ cpus). Measured the
...
/// Concrete, resolved parallelism for one index run: how many photos to detect at once and
/// how many intra-op threads each ONNX session gets. The session pool is sized to
/// `parallelism` so every worker can hold a distinct session with no lock contention.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct IndexingPlan {
    /// Concurrent detect+embed calls (rayon chunk width, and the session-pool size).
...
mod tests {
    ...
    fn indexing_speed_parse_defaults_to_background() {
        ...
        assert_eq!(IndexingSpeed::parse("FULL"), IndexingSpeed::Full);
        assert_eq!(IndexingSpeed::parse("background"), IndexingSpeed::Background);
        ...
        assert_eq!(IndexingSpeed::parse(IndexingSpeed::Full.as_str()), IndexingSpeed::Full);
        assert_eq!(
    ...
    /// so `parallelism × intra ≈ cpus` (no oversubscription — the measured optimum).
    #[test]
    fn indexing_plan_scales_with_speed_and_cpus() {
        ...
        assert_eq!(
            ...
            IndexingPlan { parallelism: 2, intra_threads: 2 }
        );
        assert_eq!(
            IndexingSpeed::Background.plan(1),
            IndexingPlan { parallelism: 1, intra_threads: 1 }
        );

        // Full on the 24-thread dev machine: 2 sessions × 12 intra-op threads = the whole CPU.
        let full = IndexingSpeed::Full.plan(24);
        assert_eq!(full, IndexingPlan { parallelism: 2, intra_threads: 12 });
        assert!(full.parallelism * full.intra_threads <= 24, "no oversubscription");
    ...
    fn load_indexing_plan_reads_setting() {
        ...
        assert_eq!(plan.parallelism, IndexingSpeed::Background.plan(
