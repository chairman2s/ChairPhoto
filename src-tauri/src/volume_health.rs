...
/// The single existence check for one candidate copy. Every resolver stat — here, in
/// [`crate::catalog::Catalog::resolve_photo_path`], and in the storage planners'
/// `resolve_*`/`filter_*` halves (`catalog/lifecycle.rs`, #85) — goes through this one
/// function, so the test counter above sees all of them.
